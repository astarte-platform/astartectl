...
func convertCassandraConnectionSpec(oldSpec *unstructured.Unstructured) (newConnection *unstructured.Unstructured) {
	...
	// sslConfiguration.enabled is now sslConfiguration.enable
	sslEnabled, found, err := unstructured.NestedBool(newConnection.Object, "sslConfiguration", "enabled")
	if err != nil {
		slog.Error("error retrieving cassandra connection sslConfiguration.enabled", "err", err)
	}
	if found {
		if err := unstructured.SetNestedField(newConnection.Object, sslEnabled, "sslConfiguration", "enable"); err != nil {
			slog.Error("error setting cassandra connection sslConfiguration.enable", "err", err)
		}
		unstructured.RemoveNestedField(newConnection.Object, "sslConfiguration", "enabled")
	}
	...
	if nodes, err := migrationutils.ParseCassandraStrNodes(oldSpec); len(nodes) > 0 && err == nil {
...
func convertCassandraSpec(oldSpec *unstructured.Unstructured, nonInteractive bool) (newCassandra *unstructured.Unstructured) {
	...
	// In v1alpha3, astarte-operator managed the Cassandra deployment when spec.cassandra.deploy was true.
	// In v2alpha1, stateful services are no longer managed by the operator.
	// Therefore, if deploy was true, we must transition this to an "external" cluster connection,
	// which requires prompting the user to provide the connection credentials and node details for
	// the cluster they will now have to manually provision.
...
// convertRabbitMQConnectionSpec converts the spec.rabbitmq.connection section from v1alpha3 to v2alpha1.
// If nonInteractive is true, it proceeds with default or missing values instead of prompting for inputs.
func convertRabbitMQConnectionSpec(oldSpec *unstructured.Unstructured, nonInteractive bool) (newConnection *unstructured.Unstructured, newManagementConnection *unstructured.Unstructured) {
	...
	sslEnabled, found, err := unstructured.NestedBool(newConnection.Object, "sslConfiguration", "enabled")
	if err != nil {
		slog.Error("error retrieving rabbitmq connection sslConfiguration.enabled", "err", err)
	}
	if found {
		if err := unstructured.SetNestedField(newConnection.Object, sslEnabled, "sslConfiguration", "enable"); err != nil {
			slog.Error("error setting rabbitmq connection sslConfiguration.enable", "err", err)
		}
		unstructured.RemoveNestedField(newConnection.Object, "sslConfiguration", "enabled")
	}
...
func convertRabbitMQSpec(oldSpec *unstructured.Unstructured, nonInteractive bool) (newRabbitMQ *unstructured.Unstructured) {
	...
	// Similar to Cassandra, v2alpha1 no longer deploys RabbitMQ.
	// If the old CR relied on the operator to deploy it (deploy == true),
	// we must transition to an external connection, meaning prompting the user manually
	// to provide credentials to an external, self-managed RabbitMQ cluster.
...
// convertCsrRootCaSpec converts the spec.cfssl.csrRootCa section from v1alpha3 to v2alpha1
...
// convertCaRootConfig converts the spec.cfssl.caRootConfig section from v1alpha3 to v2alpha1
...
// In v1alpha3, Astarte generic components (like pairing, housekeeping) were composed of two parts: api and backend.
// In v2alpha1, these are unified. This function combines the environment variables of both.
// When an environment variable with the same name exists in both "api" and "backend",
// the "backend" value takes precedence and overrides the API one.
...
func mergeAdditionalEnv(oldSpec *unstructured.Unstructured, componentName string) (interface{}, bool) {
	...
	if apiEnvFound && backendEnvFound {
		...
		if mergedEnv := migrationutils.MergeAdditionalEnv(aEnvList, bEnvList); mergedEnv != nil {
			unstructuredEnv, err := migrationutils.EnvVarListToUnstructured(mergedEnv)
			if err != nil {
				slog.Error("error converting merged additionalEnv to unstructured", "err", err)
				return nil, false
			}
			return unstructuredEnv, true
		}
	}
...
func mergeResources(oldSpec *unstructured.Unstructured, componentName string) (interface{}, bool, error) {
	...
	if apiFound && backendFound {
		// Both API and Backend components existed as separate stateless pods in v1alpha3.
		// In v2alpha1, they are unified into a single workload pod per domain (e.g. one pod for Pairing).
		// We sum their resource requests and limits to ensure the new unified pod
		// has enough resources allocated to handle both previous workloads simultaneously.
		aRR, errA := migrationutils.UnstructuredToResourceRequirements(aRes)
		if errA != nil {
			slog.Error("error converting api resources", "err", errA)
			// Non-fatal error because we still want to try to keep backend resources if conversion fails
		}
		bRR, errB := migrationutils.UnstructuredToResourceRequirements(bRes)
		if errB != nil {
			slog.Error("error converting backend resources", "err", errB)
			// Non-fatal error because we still want to try to keep api resources if conversion fails
		}
...
func convertAstarteGenericComponentSpec(oldSpec *unstructured.Unstructured, componentName string) (newComponent *unstructured.Unstructured) {
	...
	// If only api found, copy it entirely to newComponent (shallow map iteration)
	// Even though this is a shallow structural copy, it's sufficient here because the nested items usually aren't mutated
	// afterwards during this translation step, and Astarte generic components mostly contain scalar fields or simple structs.
	...
	// If only backend found, copy it entirely to newComponent (shallow map iteration)
	// Similar shallow mapping approach as the API-only copy strategy above.
...
// RunConversion converts a v1alpha3 Astarte CR to a v2alpha1 CR.
// When nonInteractive is true, missing fields are filled with placeholder/default values
// instead of prompting the user.
func RunConversion(oldCr *unstructured.Unstructured, nonInteractive bool) (*unstructured.Unstructured, error) {
	...
	// Update API version
	if err := unstructured.SetNestedField(newCr.Object, "api.astarte-platform.org/v2alpha1", "apiVersion"); err != nil {
		return nil, fmt.Errorf("error setting apiVersion in new CR: %w", err)
	}
	...
	if err := unstructured.SetNestedField(newCr.Object, kind, "kind"); err != nil {
		return nil, fmt.Errorf("error setting kind in new CR: %w", err)
	}
	...
	if err := unstructured.SetNestedField(newCr.Object, name, "metadata", "name"); err != nil {
		return nil, fmt.Errorf("error setting metadata.name in new CR: %w", err)
	}
	...
	if found {
		if err := unstructured.SetNestedField(newCr.Object, namespace, "metadata", "namespace"); err != nil {
			return nil, fmt.Errorf("error setting metadata.namespace in new CR: %w", err)
		}
	}

	// Set the new Astarte Version
	if err := unstructured.SetNestedField(newCr.Object, "1.3.0", "spec", "version"); err != nil {
		return nil, fmt.Errorf("error setting spec.version in new CR: %w", err)
	}
	...
	if convertedSpec != nil {
		...
		if err := unstructured.SetNestedField(newCr.Object, convertedSpec.Object, "spec"); err != nil {
			return nil, fmt.Errorf("error setting spec in new CR: %w", err)
		}
	} else {
		return nil, fmt.Errorf("spec section is missing or empty in the input CR. Resulting CR is invalid")
	}
